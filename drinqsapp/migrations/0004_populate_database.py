...
def populateDatabase(df1, df23):
    # glasses
    glasses = pd.Series(map(lambda x: str(x).title() if isinstance(x, str) else x, df1.glass.append(df23.glass))).dropna().drop_duplicates().sort_values(ignore_index=True)
    ...
    for cocktail in df1.iloc:
        ...
        for i in range(1, sum('ingredient' in col for col in df1.columns) + 1):
            if isinstance(cocktail[f'ingredient{i}'], str):
                ...
                CocktailIngredient.objects.create(measurement=measurement, position=i, cocktail=cocktailObject, ingredient=ingredientObject).save()

def populate(apps, schema_editor):
    df1, df2, df3 = importDatasets('data/all_drinks.csv', 'data/hotaling_cocktails.csv', 'data/hotaling_cocktails2.csv')
    cleanDatasets(df1 = df1, df2 = df2, df3 = df3)
    df23 = joinDatasets(df2 = df2, df3 = df3)
    populateDatabase(df1 = df1, df23 = df23)
